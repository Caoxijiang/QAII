...
<div class="tool">
	<div class="layui-btn-group demoTable">
  	  <span class="limit">个人获奖情况( <span id="countnum"> </span> )</span>
		<a href="javascript:;" onclick="srchange('personAdd.do')">
	...
	<div class="demoTable" style="float: right;margin-right: 115px;">
		<select class="search" id="switch">
			...
			<option value="personName">获奖人员</option>
...
<div class="act">
	...
	<div class="int-inline"><input id="personName"  type="checkbox" value="获奖人员" checked/><lable>获奖人员</lable></div>
...
layui.use('table', function(obj){
  ...
  table.render({
	...
	id: 'testReload',  
//    height: 332,
    url: 'listAwardPersonals.do', //数据接口
    ...
    cols: [[ //标题栏
		...
		{field: 'personName', title: '获奖人员',sort: true,width:180},
  		...
  		//全局搜索
  		 ...
  		 alert("搜索'全部'列，中含有关键字'"+key+"'数据，共计'"+numb+"'条！");
			  ...
			  if(check=="awardLevel"){
				  ...
				  alert("搜索'获奖人员'列，中含有关键字'"+key+"'数据，共计'"+numb+"'条！");
...
//  	window.location.reload();//刷新当前页面.
  ...
  table.on('tool(demo)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
    ...
    if(layEvent === 'detail'){
    	var iframesrc="personCheck.do?userId='"+data.id+"'";
      ...
      layer.confirm('确定删除信息', function(index){
    	...
    	var iframesrc="personEdit.do?userId='"+data.id+"'";
    	$("body", parent.document).find('iframe').attr('src',iframesrc);
    }
  });
    ...
    var $ = layui.$, active = {
        delmore: function () { //获取选中数据
            ...
            if(arr.length!=0){
                dell("deleteAwardPersonal.do",arr);
            }else {
...
upload.render({
  elem: '#test3'
  ,url: 'insertAwardPersonalWithExcel.do'
  ,accept: 'file' //普通文件
  ,done: function(res){
	  alert("上传成功！请更新数据！");
    console.log(res)
