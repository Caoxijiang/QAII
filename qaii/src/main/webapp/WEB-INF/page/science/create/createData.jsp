...
<div class="tool">
	<div class="techadd">
		...
		<span class="blue">平台建设&nbsp;—&nbsp;资料审查</span>
	</div>
...
<script src="${basePath}/js/createdata.js"></script><!-- 添加流程 -->
...
var userID="";
userID=${param.userId};
var govplatName="${requestScope.utflist[0]}";
$(function(){
	$(document).on('click','#test3',function(){
		layui.use('layer', function(){
			  ...
			  layer.open({
				type: 2, 
				area: ['350px', '400px'],
				content: "createDataAdd.do?Id"+'='+${param.userId}
			});
		}); 
	});
	$(document).on('click','.createFile',function(){
		...
		layui.use('layer', function(){
			  ...
			  layer.open({
				type: 2, 
				area: ['600px', '450px'],
				content: 'createDataAddFile.do?userId='+userID+"&step='"+stepnum+"'&govplatName='"+govplatName+"'"
			});
		}); 
	});
