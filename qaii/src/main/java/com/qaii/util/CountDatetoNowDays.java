...
public class CountDatetoNowDays {
	...
	//返回参数日期加上addDay天的日期
	public static String DateaddDays(String date,int addDay) {
		SimpleDateFormat sdf=new SimpleDateFormat("yyyy/MM/dd");
		Calendar cal=Calendar.getInstance();
	...
	//获取目标当月1日的String型日期以及上个月1日的String日期，本月的key值为this，上月的为last
		public static Map<String,String> SgetfistDay(Date date) throws ParseException {
			Map<String, String> map=new HashMap<>();
			SimpleDateFormat sdf=new SimpleDateFormat("yyyy/MM/dd");
			Calendar cal1=Calendar.getInstance();
			cal1.setTime(date);
	...
	public static Map<String , String> FirstandEndDayofYear(String date) {
	...
	//获取包括当前时间的前几个月
	public static List<String> getpremonth(String date,int add){
	...
	//获取包括当前年的前几年日期
	public static List<String> getpreYears(String date,int add){
		List<String> result=new ArrayList<>();
		Date dat=new Date(date);
		SimpleDateFormat sdf=new SimpleDateFormat("yyyy/MM/dd");
		Calendar cal=Calendar.getInstance();
